---
title: "Writing a client with biohttp"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Writing a client with biohttp}
...
| --- | --- | --- |
| `BIOHTTP_CACHE_TTL` | 1800 | Default lifetime of an entry, seconds |
...
The lifetime is per entry, not per store. `BIOHTTP_CACHE_TTL` is what an entry
gets when the call did not say otherwise, and `get_json()`, `post_json()`,
`get_text()`, and their `_many()` counterparts take a `ttl` in seconds to
override it for that one call. The case this exists for is a bulk table: a file
that took a minute to download should not expire on the same clock as a
one-record lookup, so pass `ttl = 86400` on that call and leave the default
alone for everything else. An entry past its time is a miss and is dropped on
the next read. The disk tier's `BIOHTTP_CACHE_DISK_TTL` still caps how long a
file survives on disk, whatever the call asked for.
